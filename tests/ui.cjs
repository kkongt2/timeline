...
assert(nodes['#dateCalendar'].innerHTML.includes('data-date="20300131"'));assert(nodes['#dateCalendar'].innerHTML.includes('data-date="20300201"'));assert(!nodes['#dateCalendar'].innerHTML.includes('20300202'));
nodes['#raceButtons'].click({target:{closest:()=>({dataset:{race:'2'}})}});assert.equal(vm.runInContext('current.race_no',sandbox),2);
nodes['#dateCalendar'].click({target:{closest:()=>({dataset:{date:'20300201'}})}});assert.equal(vm.runInContext('current.race_no',sandbox),3);assert.equal(nodes['#date'].value,'2030-02-01');
