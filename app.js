...
{number:2,name:'샘플B',rating:49,burden:53.5,starts_1y:8,wins_1y:1,seconds_1y:2,thirds_1y:2,distance_starts:3,distance_top3:2,recent_finishes:[3,2,4,3,6],jockey:'기수B'},
{number:3,name:'샘플C',rating:45,burden:54,starts_1y:10,wins_1y:1,seconds_1y:1,thirds_1y:1,distance_starts:5,distance_top3:1,recent_finishes:[7,5,3,8,4],jockey:'기수C'},
{number:4,name:'샘플D',rating:51,burden:56,starts_1y:7,wins_1y:2,seconds_1y:0,thirds_1y:1,distance_starts:2,distance_top3:1,recent_finishes:[1,6,2,7,5],jockey:'기수D'},
{number:5,name:'샘플E',rating:42,burden:52,starts_1y:11,wins_1y:0,seconds_1y:2,thirds_1y:1,distance_starts:4,distance_top3:1,recent_finishes:[5,4,6,3,7],jockey:'기수E'},
{number:6,name:'샘플F',rating:47,burden:54.5,starts_1y:6,wins_1y:1,seconds_1y:1,thirds_1y:1,distance_starts:2,distance_top3:1,recent_finishes:[4,2,8,5,3],jockey:'기수F'},
...
$('#demoBtn').onclick=()=>render(rank({...DEMO,venue,venue_name:venueName[venue],date:$('#date').value.replaceAll('-',''),race_no:+$('#raceNo').value}));
$('#loadBtn').onclick=async()=>{try{let {rs}=await latest(),t=rs.find(x=>(x.venue===venue||x.venue_name===venueName[venue])&&String(x.date).replaceAll('-','')===$('#date').value.replaceAll('-','')&&+x.race_no===+$('#raceNo').value);if(!t)throw 0;selectRace(t)}catch{alert('선택한 경주 자동 데이터가 아직 없습니다.')}};
$('#nextBtn').onclick=async()=>{try{let {rs}=await latest(),now=Date.now(),future=rs.filter(x=>raceTime(x)>=now-2*60*1000).sort((a,b)=>raceTime(a)-raceTime(b));let same=future.find(x=>x.venue===venue),t=same||future[0];if(!t)throw 0;selectRace(t)}catch{alert('예정된 자동 경주 데이터를 찾지 못했습니다.')}};
latest().then(({rs})=>{let future=rs.filter(x=>raceTime(x)>=Date.now()-2*60*1000).sort((a,b)=>raceTime(a)-raceTime(b));if(future[0])selectRace(future[0])}).catch(()=>{$('#dataStatus').textContent='자동 데이터 연결 실패 · 수동/데모 사용 가능'});
$('#applyOddsBtn').onclick=()=>cur?render(rank(cur,{place:od($('#placeOdds').value),qpl:od($('#qplOdds').value)})):alert('먼저 데이터를 불러오세요.');$('#manualBtn').onclick=()=>{try{render(rank(JSON.parse($('#manualJson').value)))}catch{alert('JSON 형식을 확인하세요.')}};
$('#lockBtn').onclick=()=>$('#lockScreen').classList.remove('hidden');$('#unlockBtn').onclick=()=>{if($('#pinInput').value==='1234')$('#lockScreen').classList.add('hidden');else alert('PIN이 다릅니다.')};